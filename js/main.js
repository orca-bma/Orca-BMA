...
$(function () {
    ...
    ***************************/
    const options = {
        ...
        linkSelector: 'a[href:not([target="_blank"]):not([href^="mailto:"]):not([href^="tel:"]):not([herf^="#"]):not([data-no-swup])]',
        animationSelector: '[class="mil-main-transition"]'
    };

    const isHttpProtocol = /^https?:$/.test(window.location.protocol);
    if (isHttpProtocol) {
...
// Smooth navigation with working preloader animation
$(document).on("click", ".mil-menu-frame a, .mil-main-menu a", function (e) {
    ...
    if (
        ...
        link.startsWith("tel:") ||
        $(this).is('target="_blank"], [data-no-swup]')
    ) {
    ...
    if (window._swup) {
        e.preventDefault();
        $(".mil-preloader").removeClass("mil-hidden").fadeIn(300);

        setTimeout(function () {
            window._swup.loadPage({url: link, method: 'GET'});
        }, 300);
        return;
    }
   ...
   $(document).on('click', 'a[href^="#"]', function (event) {
    ...
    // Only run smooth scroll if href is a real ID inside the same page
